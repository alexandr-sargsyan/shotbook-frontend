...
- **Фильтры**: 
  - Категория (множественный выбор)
  - Платформа (YouTube, Instagram, TikTok)
  - Темп (медленный, быстрый, смешанный)
  - Уровень продакшена
  - Теги (поиск с автодополнением, множественный выбор, логика OR)
...
- **Сетка видео**: 
  - 6 колонок на больших экранах (> 1024px)
  - Адаптивная сетка для планшетов и мобильных
  - Lazy loading видео через Intersection Observer
  - Активные видео с autoplay, muted, loop в списке
  - Вертикальное соотношение сторон 9:16 (как в TikTok)
  - Тайтл: тонкий курсив 13px, фиксированная высота блока, обрезка длинных текстов
...
- Адаптивная сетка видео (6 колонок на больших экранах)
- Вертикальные видео-карточки (9:16 aspect ratio, TikTok-style)
- Тонкий курсивный текст тайтлов (13px, font-weight: 300)
- Фиксированная высота блока тайтла с обрезкой длинных текстов
...
│   ├── VideoGrid/            # Сетка видео (6 колонок)
│   ├── VideoCard/            # Карточка видео с lazy loading
│   ├── SearchBar/            # Поисковая строка
│   ├── FilterSidebar/        # Панель фильтров (включая теги)
│   ├── VideoPlayer/          # Видео-плееры (UnifiedVideoPlayer, YouTubePlayer, TikTokPlayer, InstagramPlayer)
...
- `GET /api/video-references` - Поиск и фильтрация видео
  - Параметры: `search`, `category_id`, `platform`, `pacing`, `production_level`, `tag_ids[]`, `has_*`, `sort_by`, `page`, `per_page`
- `GET /api/video-references/:id` - Получить видео по ID
- `GET /api/categories` - Список категорий (с дочерними категориями)
- `GET /api/tags` - Список тегов
  - Параметр `search` - поиск тегов по имени
